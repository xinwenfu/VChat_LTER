...
def main():
    ...
    # Define the fuzzing structure for the "LTER" command
    s_initialize("LTER")  # Initialize a fuzzing block named "LTER"
    s_string('LTER', fuzzable=False, name='LTER-Command')  # Send the command "LTER" (not fuzzable)
    s_delim(' ', fuzzable=False, name='LTER-Space')  # Send a space after "LTER" (not fuzzable)
    s_string('A', name='LTER-STRING')  # Fuzz this part by sending different "A" variations
    s_static('\r\n', name='LTER-CRLF')  # Append a carriage return + newline to mimic real commands

    session.pre_send = receive_banner  # Assign `receive_banner` as a pre-send callback to read any initial response

    session.connect(s_get("LTER"))  # Connect the fuzzing session to the "LTER" structure defined above
